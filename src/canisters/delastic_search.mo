...
actor ElasticSearch {
	...
	private func retrieveRecords (tokens: [Text], entityType: Text) : [Types.Record] {
	...
	public query func queryIndex (queryString: Text, entityType: Text) : async Result.Result<[Types.Record], Types.Error> {
		...
		if (Text.contains(queryString, #char ' ')) {
			...
			let relevantRecords = retrieveRecords(Iter.toArray(tokens), entityType);
			return #ok(relevantRecords);
		} else {
			// edge case for no spaces
			let relevantRecords = retrieveRecords([queryString], entityType);
			return #ok(relevantRecords);
		};
	};

	public func addRecord (newRecord: Types.Record, indexKeys: [Text], oldIndexKeys: [Text]) : async () {
